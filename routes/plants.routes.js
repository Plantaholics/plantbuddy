...
router.get("/plants", (req, res, next) => {
  Plant.find()
    ...
    .catch(err => res.status(500).json({ error: "Error fetching plants", details: err }));
});
...
router.get("/plants/:plantId", (req, res, next) => {
  ...
  Plant.findById(plantId)
    ...
    .catch(err => res.status(500).json({ error: "Error fetching plant", details: err }));
});
...
router.put("/plants/:plantId", isAuthenticated, (req, res, next) => {
  ...
  Plant.findOneAndUpdate({ _id: plantId, createdBy: userId }, req.body, { new: true })
    .then(updatedPlant => {
      if (!updatedPlant) {
        res.status(403).json({ message: "You are not authorized to edit this plant" });
        return;
      }
      res.json(updatedPlant);
    })
    .catch(err => res.status(500).json({ error: "Error updating plant", details: err }));
});
...
router.delete("/plants/:plantId", isAuthenticated, (req, res, next) => {
  ...
  Plant.findOneAndDelete({ _id: plantId, createdBy: userId })
    .then(deletedPlant => {
      if (!deletedPlant) {
        res.status(403).json({ message: "You are not authorized to delete this plant" });
        return;
      }
      res.json({ message: `Plant with ${plantId} has been removed successfully` });
    })
    .catch(err => res.status(500).json({ error: "Error deleting plant", details: err }));
});
